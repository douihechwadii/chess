...
void UI_DrawHighlights(const UIState *ui, const GameState *state)
{
    ...
    if (GameState_IsInCheck(state))
    {
        ...
        if (kSq >= 0)
            DrawRectangleRec((Rectangle){UI_SquareToScreen(kSq).x, UI_SquareToScreen(kSq).y, SQUARE_SIZE, SQUARE_SIZE}, COLOR_CHECK);
    }
}
...
void UI_DrawPieces(const UIState *ui, const GameState *state)
{
    for (int i = 0; i < BOARD_SIZE; i++)
    {
        ...
        if (p != PIECE_NONE)
        {
            ...
            if (texIdx >= 0)
                DrawTexture(ui->pieces[texIdx], UI_SquareToScreen(i).x + 5, UI_SquareToScreen(i).y + 5, WHITE);
        }
    }
}
...
bool UI_HandleInput(UIState *ui, GameState *state)
{
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
    {
        ...
        if (hovered >= 0)
        {
            if (ui->selectedSquare >= 0)
            {
                ...
                if (m)
                {
                    if (m->flags & MOVE_PROMOTION)
                        m->promotion = UI_PromotionDialog(ui, state->sideToMove);
                    ui->lastMove = *m;
            ...
            if (p != PIECE_NONE && Board_PieceColor(p) == state->sideToMove)
                ui->selectedSquare = hovered;
            else
                ui->selectedSquare = -1;
        }
    }
